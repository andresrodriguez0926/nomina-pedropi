...
// 1. Fix Christmas Salary Calculation filter
...
if(content.includes(targetStr)) {
    ...
    content = content.replace(`if (sameId) {`, `if (sameId || sameReg) {`);
}

// 3. Add lastChristmasData
content = content.replace(
    '    });\n\n    container.innerHTML = `',
    '    });\n\n    window.lastChristmasData = christmasData;\n    container.innerHTML = `'
);

// 4. Inject buttons safely using indexOf, WITH PROPER NEWLINES
...
// 5. Inject the whole renderChristmasReportByDepartment block before quickAddChristmasSalary
const reportFuncStr = `
