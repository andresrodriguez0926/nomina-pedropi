...
// 2. Insert window.lastChristmasData = christmasData;
const htmlStart = '    container.innerHTML = `\n            <div class="header-action no-print">\n            <h1>Salario de Navidad (Regalía Pascual)</h1>';
const htmlReplacement = '    window.lastChristmasData = christmasData;\n' + htmlStart;
if (content.includes(htmlStart)) {
    content = content.replace(htmlStart, htmlReplacement);
    console.log('Inserted lastChristmasData assignment successfully.');
} else {
    console.error('Could not find HTML start for Salario de Navidad.');
}

// 3. Add the button to renderChristmasSalary
const buttonHtml = `
...
`;
if (content.includes('<div class="action-group" style="gap: 10px">')) {
    content = content.replace('<div class="action-group" style="gap: 10px">', buttonHtml);
    console.log('Added Reporte por Depto button successfully.');
}

// 4. Inject the whole renderChristmasReportByDepartment block before quickAddChristmasSalary
const code = `
...
if (insertIdx !== -1) {
    ...
    fs.writeFileSync('app.js', before + code + '\n' + after, 'utf8');
    console.log("Updated app.js successfully!");
} else {
    console.error("Could not find quickAddChristmasSalary to insert before!");
}
